...
var Joyride = function (_React$Component) {
  ...
  _createClass(Joyride, [{
    ...
    key: 'getScrollLeft',
    value: function getScrollLeft(baseLeftPostition) {
      // get offset X of hole
      var index = this.state.index;
    ...
    value: function calcPlacement() {
      var _state8 = this.state,
          index = _state8.index,
          isRunning = _state8.isRunning,
          standaloneData = _state8.standaloneData;
      ...
      if (step && (standaloneData || isRunning && steps[index])) {
        var clientWidth = Math.max(document.body.clientWidth, 1280);
        var clientHeight = Math.max(document.body.clientHeight, 768);
        var isVirtualTarget = /^.tour-guide__placehold/.test(steps[index].selector);
        var scrollTop = isVirtualTarget ? 0 : this.getScrollTop() - 85;
        var offsetParent = document.querySelector((0, _utils.sanitizeSelector)(offsetParentSelector));
        if (!target) return;
        var rect = (0, _utils.getOffsetBoundingClientRect)(target, offsetParent);
        var popupHeight = 200;
        var popupWidth = 610;
        var paddingPopup = 20;
        var supperButtonBounding = document.querySelector('#super-button').getBoundingClientRect();
        if (/^ls_pd2/.test(steps[index].casecClass)) {
          var scrollLeft = isVirtualTarget ? 0 : this.getScrollLeft(clientWidth / 2 + 10);
          ...
          placement.x = rect.left + rect.width + 2 * paddingPopup;
          placement.y = rect.top - 2 * paddingPopup;
    ...
    value: function getRenderStage() {
      var _state9 = this.state,
          shouldRedraw = _state9.shouldRedraw,
          xPos = _state9.xPos;
    ...
    value: function createComponent() {
      var _state10 = this.state,
          index = _state10.index,
          shouldRedraw = _state10.shouldRedraw,
          shouldRenderTooltip = _state10.shouldRenderTooltip,
          standaloneData = _state10.standaloneData,
          xPos = _state10.xPos,
          yPos = _state10.yPos;
